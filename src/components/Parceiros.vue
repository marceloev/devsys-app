<template>
    <div id="app" class="container-fluid">
        ...
        <div id="navigator" class="row">
            ...
            <div class="col p-0">
                <el-input v-model="filtro" @change="filtrar()" maxlength="50" placeholder="Pesquise aqui algo específico ;)" prefix-icon="el-icon-search" clearable/>
            </div>
            <div class="col-auto m-0">
                <el-tag type="info" size="medium" effect="plain" color="whitesmoke" 
                    class="font-weight-bold h-100 m-l-0" data-toggle="tooltip" data-placement="left" title="Qtd. de Linhas na tabela">
                    <u>{{ this.parceiros.length }}</u>
                </el-tag>
            </div>
        </div>
        ...
        <div id="modo-grade" v-show="!modoFormulario">
            <el-table ref="tabela" :data="parceiros" border style="width: 100%" size="mini" stripe resizable highlight-current-row @current-change="currentLineChanged" @row-dblclick="changeLayout()" :default-sort="{prop: 'codigo'}">
...
export default {
  ...
  methods: {
    ...
    novo() {
      ...
      let filter = this.filtro.toUpperCase();
      let tabela = this.$refs.tabela.$el;
      let rowSize = this.parceiros.length;
      let colSize = tabela.getElementsByTagName("tbody")[0].getElementsByTagName("tr")[0].getElementsByTagName("td").length;
      
      /*for (var row = 0; row < tr.length; i++) {
